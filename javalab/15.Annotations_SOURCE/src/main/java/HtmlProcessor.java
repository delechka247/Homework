    ...
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        ...
        for (Element element : annotatedElements) {
            ...
            try {
                ...
                Set<? extends Element> annotatedElementsForInput = roundEnv.getElementsAnnotatedWith(HtmlInput.class);
                for (Element elementForInput : annotatedElementsForInput) {
                    HtmlInput annotationForInput = elementForInput.getAnnotation(HtmlInput.class);
                    writer.write("\t<input type='" + annotationForInput.type() + "' name='" + annotationForInput.name() + "' placeholder='" + annotationForInput.placeholder() + "'>\n");
                }
                writer.write("</form>");
                writer.close();
            } catch (IOException e) {
